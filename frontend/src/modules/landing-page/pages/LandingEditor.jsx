...
const LandingEditor = () => {
        ...
        if (id) {
          ...
          if (pageRes.data.success) {
            ...
            setPageData({
              ...
              theme: data.theme || { primaryColor: '#4f46e5', backgroundColor: '#ffffff', textColor: '#0f172a' },
  ...
  const selectTemplate = (t) => {
    setPageData({ 
      ...
      theme: t.theme || { primaryColor: '#4f46e5', backgroundColor: '#ffffff', textColor: '#0f172a' }, 
      templateId: t.id 
    });
  };
    ...
    <div className="h-screen flex flex-col bg-slate-950 overflow-hidden font-sans antialiased">
      ...
      <header className="h-16 bg-slate-950 border-b border-white/[0.05] flex items-center justify-between px-6 shrink-0 z-50">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-2 text-slate-500 hover:text-white bg-white/[0.03] border border-white/5 rounded-xl transition-all">
            {isSidebarCollapsed ? <PanelLeftOpen size={18}/> : <PanelLeftClose size={18}/>}
          </button>
          <div className="h-6 w-px bg-white/10" />
          <button onClick={() => navigate(`/${tenant?.slug}/landings`)} className="p-2 text-slate-400 hover:text-white bg-white/[0.03] border border-white/5 rounded-xl transition-all flex items-center gap-2 group">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleSave} className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-2xl shadow-indigo-500/20 transition-all active:scale-95">Publicar Ahora</button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative">
                     ...
                     <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                           {['reversed', 'split', 'centered'].map(l => (
                             <button key={l} onClick={() => updateSectionContent('layout', l)} className={`py-2 rounded-lg border text-[8px] font-black uppercase transition-all ${pageData.sections[selectedSectionIndex].content.layout === l ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white/5 border-white/5 text-slate-500 hover:text-white'}`}>{l}</button>
                           ))}
                        </div>
                     </div>
                   )}
              ...
              <div className="space-y-12">
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {[['primaryColor', 'Primario'], ['backgroundColor', 'Fondo'], ['textColor', 'Texto']].map(([key, label]) => (
                      <div key={key} className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/[0.05] rounded-xl">
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{label}</span>
                        <input type="color" value={pageData.theme?.[key] || '#000'} onChange={(e) => updateTheme(key, e.target.value)} className="w-8 h-8 rounded-lg bg-transparent border-none cursor-pointer" />
                      </div>
                    ))}
                  </div>
                </div>
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    {pageData.sections.map((s, idx) => (
                      <button key={idx} onClick={() => setSelectedSectionIndex(idx)} className={`w-full p-4 rounded-xl border transition-all flex items-center justify-between group ${selectedSectionIndex === idx ? 'border-indigo-600 bg-indigo-600/10' : 'border-white/[0.03] bg-white/[0.01] hover:border-white/[0.08] hover:bg-white/[0.03]'}`}>
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-[0.1em]">{s.type}</span>
                        </div>
                        <ChevronRight size={14} className={`${selectedSectionIndex === idx ? 'text-indigo-600' : 'text-slate-700'}`} />
                      </button>
                    ))}
                  </div>
                </div>
        ...
        <main className="flex-1 overflow-auto bg-slate-950 custom-scrollbar scroll-smooth flex flex-col items-center">
          <div className="relative py-20 px-8 lg:px-20 min-w-max flex flex-col items-center">
            <div className="bg-white shadow-[0_100px_200px_-50px_rgba(0,0,0,0.8)] rounded-[3rem] overflow-hidden border border-white/[0.05] relative origin-top transition-all duration-700 ease-out"
              style={{ transform: `scale(${zoom})`, width: '1280px', marginBottom: `calc(1280px * (1 - ${zoom}) * -0.5)` }}>
              <div className="h-12 bg-slate-100 border-b border-slate-200 flex items-center px-8 gap-3 shrink-0">
                 <div className="flex gap-2"><div className="w-3.5 h-3.5 rounded-full bg-red-400" /><div className="w-3.5 h-3.5 rounded-full bg-yellow-400" /><div className="w-3.5 h-3.5 rounded-full bg-green-400" /></div>
                 <div className="mx-auto bg-white border border-slate-200 px-8 py-1.5 rounded-full text-[10px] font-black text-slate-400 tracking-widest flex items-center gap-3">
                    <Globe size={14} className="text-indigo-400" /> {tenant?.slug}.modular.com/{pageData.path}
                 </div>
              </div>
