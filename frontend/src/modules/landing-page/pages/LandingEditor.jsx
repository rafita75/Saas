...
import { Save, ArrowLeft, Layout, Settings, Eye, Check, ChevronRight, Type, Edit3, Globe, Plus, Trash2, Smartphone, Monitor, X, Mail, Phone, Instagram, Facebook, Twitter } from 'lucide-react';
...
const LandingEditor = () => {
  ...
  const addExtraSection = (type) => {
    ...
    const defaultContent = {
      ...
      cta: { title: 'Llamada a la acción', description: 'Únete hoy', buttonText: 'Click aquí', action: { type: 'link', value: '#' } },
      testimonials: { title: 'Opiniones', items: [{ title: 'Cliente', description: 'Excelente' }] }
    };
    setPageData({ ...pageData, sections: [...pageData.sections, { id: `${type}-${Date.now()}`, type, content: defaultContent[type] || {}, order: pageData.sections.length }] });
  };

  const deleteSection = (index) => {
    if (subscription?.planId?.slug === 'gratis' && pageData.sections.length <= 4) return setToast({ message: 'No puedes borrar bloques base.', type: 'warning' });
  ...
  if (isNew && pageData.sections.length === 0) return (
    <div className="min-h-screen bg-dark-950 p-8 lg:p-12 animate-fade-in space-y-12">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
        {LANDING_TEMPLATES.map((t) => (
          <div key={t.id} className="group relative glass rounded-[48px] border border-white/5 overflow-hidden flex flex-col hover:border-primary/5 transition-all cursor-pointer shadow-2xl" onClick={() => selectTemplate(t)}>
            <div className="aspect-[4/3] bg-dark-800 flex items-center justify-center relative">
               ...
               <div className="absolute inset-0 bg-dark-950/80 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center p-8 backdrop-blur-sm"><span className="text-white font-black text-xl italic uppercase">Elegir Diseño</span></div>
            </div>
            <div className="p-10 space-y-4"><h3 className="text-2xl font-black text-white tracking-tight uppercase italic">{t.name}</h3><p className="text-slate-400 text-sm leading-relaxed">{t.description}</p></div>
          </div>
        ))}
      </div>
    </div>
  ...
  const isFree = subscription?.planId?.slug === 'gratis';
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col gap-6 animate-fade-in font-sans selection:bg-primary/30 antialiased">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
        <div className="flex items-center gap-6">
          ...
          <div><input type="text" value={pageData.name} onChange={(e) => setPageData({...pageData, name: e.target.value})} className="bg-transparent text-2xl font-black text-white outline-none border-b-2 border-transparent focus:border-primary/50 transition-all" placeholder="Nombre..." /></div>
        </div>
        <div className="flex gap-4">
          <div className="hidden lg:flex bg-dark-900/50 p-1 rounded-2xl border border-white/5 backdrop-blur-xl">
            {[{ id: 'editor', label: 'Editor', icon: Layout }, { id: 'settings', label: 'SEO', icon: Globe }].map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-8 py-3 rounded-2xl text-xs font-black uppercase transition-all flex items-center gap-3 ${activeTab === tab.id ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-slate-200'}`}><tab.icon size={16} /> {tab.label}</button>
            ))}
          </div>
                ...
                <div className="space-y-8 animate-in slide-in-from-left">
                  ...
                  <div className="space-y-6">
                    {Object.keys(selectedSection.content).map((key) => {
                      ...
                      return (<div key={key} className="space-y-2"><label className="text-[10px] font-black text-slate-600 uppercase ml-2">{key}</label>{key.toLowerCase().includes('description') ? <textarea value={val} onChange={(e) => updateSectionContent(key, e.target.value)} className="w-full bg-dark-800 border border-slate-700 rounded-3xl px-5 py-4 text-sm text-slate-300 outline-none h-40 resize-none" /> : <input type="text" value={val} onChange={(e) => updateSectionContent(key, e.target.value)} className="w-full bg-dark-800 border border-slate-700 rounded-[20px] px-5 py-4 text-sm text-slate-300 outline-none" />}</div>);
                    })}
                    {(selectedSection.content.ctaText || selectedSection.content.buttonText) && (
                      <div className="pt-8 border-t border-white/5 space-y-6">
                        <h4 className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">Interacción</h4>
                        <div className="space-y-4">
                          <input type="text" value={selectedSection.content.ctaText || selectedSection.content.buttonText} onChange={(e) => updateSectionContent(selectedSection.content.ctaText ? 'ctaText' : 'buttonText', e.target.value)} className="w-full bg-dark-800 border border-slate-700 rounded-2xl px-5 py-3 text-sm text-white" placeholder="Texto botón" />
                          <select value={selectedSection.content.action?.type || 'link'} onChange={(e) => updateAction('type', e.target.value)} className="w-full bg-dark-800 border border-slate-700 rounded-2xl px-5 py-3 text-sm text-white appearance-none"><option value="link">Enlace o Ancla (#id)</option><option value="whatsapp">WhatsApp Directo</option><option value="phone">Llamada</option><option value="email">Email</option></select>
                          <input type="text" value={selectedSection.content.action?.value || ''} onChange={(e) => updateAction('value', e.target.value)} className="w-full bg-dark-800 border border-slate-700 rounded-2xl px-5 py-3 text-sm text-white font-mono" placeholder="URL o #ID" />
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="lg:col-span-3 glass rounded-[48px] border border-white/5 bg-dark-900/30 overflow-y-auto relative shadow-inner antialiased p-8 lg:p-20">
          ...
          {activeTab === 'settings' && (
            <div className="max-w-2xl mx-auto py-20 px-8 space-y-16">
              <h3 className="text-5xl font-black text-white tracking-tighter italic uppercase">SEO</h3>
              <div className="space-y-10">
